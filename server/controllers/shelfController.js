...
const { Shelf, ProductShelf, Product } = require('../models');
...
exports.getShelfStats = async (req, res) => {
  try {
    ...
    const fullShelves = await Shelf.countDocuments({ isfull: true, isDelete: false });
    const availableShelves = await Shelf.countDocuments({ isfull: false, isDelete: false });
    
    const byCategory = await Shelf.aggregate([
      { $match: { isDelete: false } },
      { $group: { _id: '$category', count: { $sum: 1 }, totalCapacity: { $sum: '$capacity' } } }
    ]);
...
exports.getAvailableShelves = async (req, res) => {
  try {
    ...
    const shelves = await Shelf.find(query).sort('shelf_number');
...
exports.createShelf = async (req, res) => {
  try {
    ...
    const existingShelf = await Shelf.findOne({ shelf_number, isDelete: false });
    if (existingShelf) {
...
exports.updateShelf = async (req, res) => {
  try {
    ...
    // Update fields
    if (shelf_number) shelf.shelf_number = shelf_number;
...
exports.getShelfCapacity = async (req, res) => {
  try {
    ...
    const totalQuantity = productsOnShelf.reduce((sum, ps) => sum + ps.quantity, 0);
    const availableCapacity = Math.max(0, (shelf.capacity || 0) - totalQuantity);
    const utilizationPercent = shelf.capacity > 0 
