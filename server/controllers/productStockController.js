...
exports.getAllProductStocks = async (req, res) => {
  try {
    ...
    const productStocks = await ProductStock.find(query)
      .populate({
        path: 'product_id',
        select: 'name category unit price current_stock minimum_stock_level image_link sku barcode'
      })
      .populate({
        path: 'shelf_id',
...
exports.getProductStockById = async (req, res) => {
  try {
    const productStock = await ProductStock.findById(req.params.id)
      .populate({
        path: 'product_id',
        select: 'name description category unit price current_stock minimum_stock_level maximum_stock_level image_link sku barcode'
      })
      .populate({
        path: 'shelf_id',
...
exports.getStockByProduct = async (req, res) => {
  try {
    ...
    const product = await Product.findById(req.params.productId)
      .select('name category unit current_stock minimum_stock_level maximum_stock_level');

    const totalInShelves = productStocks.reduce((sum, stock) => sum + stock.quantity, 0);
...
exports.getStockByShelf = async (req, res) => {
  try {
    ...
    const shelf = await Shelf.findById(req.params.shelfId)
      .select('shelf_number category capacity isfull');
